...
export const updateDisplay = (weatherJson, locationObj) => {
  fadeDisplay(); // first to hide cntent
  clearDisplay();
  ...
  setFocusOnSearch();
  fadeDisplay(); // end to display updated content
};
...
const getWeatherClass = (icon) => {
  const firstTwoChars = icon.slice(0, 2); // icon ='01d', result = 01
  const lastChar = icon.slice(2); // result = d
  const weatherLookup = {
    '09': 'snow',
    10: 'rain',
    11: 'rain',
...
const createElem = (elemType, divClassName, divText, unit) => {
  ...
  if (divClassName === 'temp') {
    ...
    unitDiv.className = 'unit'; // or unitDiv.classlist.add('unit')
